...
<body>
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if (isset($_POST[$ingredient_id])) {
            array_push($ingredients, $ingredient_id);
            /*if ($ingredients == "") {
    ...
    while ($row = mysqli_fetch_array($result)) {
        extract($row);
        $contains = true;
        for ($i = 0; $i < strlen($receipes_ingredient); $i++) {
            if ($receipes_ingredient[$i] == ",") {
                continue;
            }
            if (!in_array(getNumber($receipes_ingredient, $i), $ingredients)) {
                $contains = false;
                break;
            }
        }
